...
# Last updated: Alexey Larionov, 20Dec2016
...
read line1 < "${job_file}"
if [ "${line1}" != "Job description for alignment and qc, aws wes pipeline" ] 
then
  echo "" 
...
# Get start script name and scripts folder name
ec2_base_folder=$(awk '$1=="ec2_base_folder:" {print $2}' "${job_file}")
...
echo "Requested job:"
echo ""
echo "Pipeline launching script: ${start_script}"
...
# Start the job
"${start_script}" "${job_file}" "${scripts_folder}" 
